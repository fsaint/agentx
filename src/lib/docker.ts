...
const OPENCLAW_IMAGE = process.env.OPENCLAW_IMAGE || "agentx-openclaw:latest";
...
export async function getLocalContainerStatus(
...
export async function getLocalContainerPort(
  containerName: string
): Promise<number> {
  try {
    const portOutput = docker("port", containerName, "18789");
